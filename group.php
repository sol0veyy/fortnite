<?php
    ...
    $resultArticle = $mysql -> query("SELECT * FROM `article` WHERE `community_id` = '{$_GET['id']}' ORDER BY id DESC");
...
<body>
    ...
    <?php
        if ($_SESSION['user']['login']) {
            ...
            if ($group['admin'] == $_SESSION['user']['id']) {
                echo "
                    <button onclick='openCreateArticle()'>Создать запись</button>
                    <form action='vendor/create-article.php?com={$_GET['id']}' method='post' class='createArticle' id='createArticle'>
                        <label>Текст</label>
                        <textarea name='text' class='text-article'></textarea>
                        <div class='buttons-form'>
                            <input type='submit' value='Создать'>
                            <input onclick='closeCreateArticle()' type='button' value='Отмена'>
                        </div>
                    </form>
                ";
            }   
            ...
            while ($article = mysqli_fetch_array($resultArticle)) {
                ...
                <div class='article'>
                    ...
                    <div class='likes'>
                        ...
                        if ($group['admin'] == $_SESSION['user']['id']) {
                            echo "
                                <form action='vendor/del_article.php?com={$_GET['id']}&article={$article['id']}' method='post'>
